...
function calcBaseRevenue(p) {
  ...
  Object.entries(p.stocks).forEach(([sid,qty]) => { const s=GS.sectors[sid]; if(s) rev+=Math.floor(s.price/5)*qty; });
...
function calcCompanyValue(c) { return c.baseValue + c.upgrades*15 + c.level*8; }
...
function applyUpgrade(company, free=false) {
  ...
  if (!free) p.cash-=cost;
  company.upgrades++; company.revenue+=3;
